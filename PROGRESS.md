# Rack Calculator Schematic Visual Fix
**Date:** 2026-04-29

## Files modified

- **`rack_diagram.py`** — lines 3, 26-70, 80-116, 126-160, and 194-216
  - Updated only `generate_diagram()` and its local SVG helpers.
  - Restored requested presentation colors: green rack beam/columns, red pipes, grey flanges, red dimension lines.
  - Replaced dark staggered flange shadows with circular grey flange plates around each pipe.
  - Added eight visible bolt holes around each flange plate.
  - Added dashed vertical centerlines through both rack columns.
  - Changed the rack-width dimension line to run from left column centerline to right column centerline.
  - Kept the first clearance dimension from left column centerline to pipe 1 centerline and constrained the first drawn flange symbol to leave a visible gap from the left column.

## Verification

- `python -m py_compile rack_diagram.py rack_calculator.py app.py` passed.
- Rendered a sample 3-pipe SVG through `calculate_rack()` + `generate_diagram()` and confirmed bolt holes, green/red/grey color markers, and `rack width CL-CL` are present.
- No changes made to `rack_calculator.py`, pipe spacing formulas, rack width formulas, Flask routes, templates, static JS, input/output data, or support selector logic.

---

# Rack Calculator Schematic Visual Refinement
**Date:** 2026-04-29

## Files modified

- **`rack_diagram.py`**
  - Improved only the SVG rendering generated by `generate_diagram()`.
  - Added a cleaner engineering top-view style with muted rack steel, column hatching, pipe centerlines, and staggered flange plates.
  - Added visual column centerlines and changed the first top dimension to run from the left rack column centerline to the first pipe centerline.
  - Improved label treatment with small white label plates, clearer DN/rating text, red dimension lines, and staggered top dimension levels to reduce overlaps.

## Verification

- `python -m py_compile rack_diagram.py rack_calculator.py app.py` passed.
- Rendered a sample 3-pipe SVG through `calculate_rack()` + `generate_diagram()` and confirmed the new `COLUMN CL` datum and `CL-CL` first dimension are present.
- No changes made to rack width calculation, pipe spacing calculation, input/output data, support selector logic, routes, templates, or static JS.

---

# Clamped Shoe PDF Link Background Highlight Refinement
**Date:** 2026-04-28

## Files modified

- **`app.py`** — lines 8 and 211-229
  - Added URL quoting for drawing refs.
  - Added `/drawing-link/<drawing_ref>` launcher route used only by embedded PDF links.
  - The launcher opens the existing `/api/drawing/<ref>?nps=<selected_nps>` PDF route in a new browser tab via `window.open(..., "_blank")`, then returns the original tab to the clamped shoe PDF.

- **`pdf_service.py`** — lines 318-325 and 342-351
  - Updated embedded PDF link URIs to point to `/drawing-link/<ref>?nps=<selected_nps>`.
  - For each clickable support-code reference, draws a subtle amber transparent rectangle behind the original PDF text before inserting the URI annotation.
  - No replacement text is inserted; the original PDF text remains unchanged.

## Verification

- `python -m py_compile pdf_service.py app.py drawing_index.py` passed.
- Generated `SC01` and `SC08` PDFs with `nps=6`; PyMuPDF read back the expected URI annotations, zero red text overlays, and matching amber background rectangles.
- Flask test client confirmed `/drawing-link/JS-PE-DPS-0327?nps=6` returns an HTML launcher containing `window.open(..., "_blank")` and the preserved `/api/drawing/JS-PE-DPS-0327?nps=6` target.

---

# Clickable References Inside Clamped Shoe PDFs
**Date:** 2026-04-28
...
- **`app.py`** — line 184
  - Passes `request.url_root` into `get_drawing_pdf()` so PDF annotations can use absolute app URLs.

- **`drawing_index.py`** — lines 51-53 and 164-166
  - Added existing standard mappings for `CL01`, `CL02`, and `CL03`:
    - `CL01` → `JS-PE-DPS-0335`
    - `CL02` → `JS-PE-DPS-0336`
    - `CL03` → `JS-PE-DPS-0337`
  - Added NPS ranges for those existing clamp component drawings: NPS 1.5"–24".

- **`pdf_service.py`** — lines 17-19, 76-78, 210-219, 318-346, 353-357, and 392-409
  - Imports `DRAWING_INDEX` / `get_drawings()` for known support-code link targets.
  - Adds `DRAWING_PAGES` entries for existing `CL01`-`CL03` drawings.
  - Defines the SC01-SC08 drawing refs that should receive embedded links.
  - Adds URI-building and PDF reference-link helpers.
  - Keeps existing row highlighting unchanged, then inserts clickable link annotations on matching support-code text inside generated SC PDFs.

## Behavior

Only generated clamped shoe PDFs (`SC01`-`SC08`, refs `JS-PE-DPS-0342` through `JS-PE-DPS-0349`) receive embedded clickable links.

When support-code text such as `SH01`, `SH03`, `SH04`, `SH05`, `CL01`, `CL02`, or `CL03` appears inside an SC PDF, it links to the existing `/api/drawing/<ref>?nps=<selected_nps>` route. The linked drawing is generated by the existing PDF service and highlights the same selected NPS row.

## Verification

- `python -m py_compile pdf_service.py drawing_index.py app.py` passed.
- Generated `SC01`, `SC05`, and `SC08` PDFs with `nps=6`; PyMuPDF read back proper URI annotations.
- Flask test client confirmed `/api/drawing/JS-PE-DPS-0342?nps=6` returns a PDF with links, and a linked target such as `/api/drawing/JS-PE-DPS-0327?nps=6` returns successfully.
