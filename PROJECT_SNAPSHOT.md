...
- **Platform redesign (markup/CSS/assets only):** shared shell + capability rail + `⌘K` palette;
  ...
  hero (subtle plant-photo atmosphere); **Phase 2.5 professional polish pass** (independent tool
  board, tool-card outputs, single `--datum` accent, focus rings, Standards governance register).
...
1. **Tool-page polish propagation** (low-risk follow-up).
2. **Engine expansion** (pages 74–190): WL01/WL02 details, vessel/equipment clips (VC), spring
   hangers (GS/SF/UB). Quick win: auto-populate flange class from MPMS `rating`.
3. **Phase 3 remains paused and gated:** do not implement the shared **Line object**,
   cross-tool handoff, or connected workflow until explicitly re-approved.
...
4. **Approved architecture & direction** — workspace shell, capability rail, governed independent tool hub
